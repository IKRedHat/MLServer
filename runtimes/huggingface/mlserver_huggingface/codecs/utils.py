...
class EqualUtil:
    ...
    def list_equal(list1: List[Any], list2: List[Any]) -> bool:
        """
        Performs deep equality comparison between two lists with support for complex nested data types
        including PIL Images, numpy arrays, dictionaries, and nested lists.
        """
        ...
        for idx, el in enumerate(list1):
            # Recursive dictionary comparison - custom equality needed for nested structures
            if isinstance(el, dict):
                if not EqualUtil.dict_equal(el, list2[idx]):
                    return False
            # Recursive list comparison - handles nested list structures
            elif isinstance(el, list):
                if not EqualUtil.list_equal(el, list2[idx]):
                    return False
            # PIL Image comparison - requires pixel-level comparison instead of object equality
            elif isinstance(el, Image.Image):
                if not EqualUtil.pil_equal(el, list2[idx]):
                    return False
            # Numpy array comparison - uses element-wise comparison via np.array_equal
            elif isinstance(el, np.ndarray):
                if not np.array_equal(el, list2[idx]):
                    return False
            # Fallback to standard equality for primitive types and other objects
            else:
                if el != list2[idx]:
                    return False
        return True
